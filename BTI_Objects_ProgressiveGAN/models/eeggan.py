...
def sample_images_eeg(epoch, generator, latent_dim, gen_imgs, labels, main_dir, model_type):
    r, c = 4, 8
    #noise = np.random.normal(0, 1, (r * c, latent_dim))
    #sampled_labels = np.array([num for _ in range(r) for num in range(c)])
    #gen_imgs = generator.predict([noise, sampled_labels])
    ...
    dir_to_save = f"./brain_to_image/EEGgan/EEG_images/{model_type}"
    save_path = os.path.join(main_dir, dir_to_save)
...
def save_model(model, model_type, model_name):
    ...
    def save(model, model_name):
        model_path = f"./brain_to_image/bigan/saved_model/{model_type}/{model_name}.json"
        weights_path = f"./brain_to_image/bigan/saved_model/{model_type}/{model_name}_weights.hdf5"
        model_file = f"./brain_to_image/bigan/saved_model/{model_type}/{model_name}_model.hdf5"
        if not os.path.exists(f"./brain_to_image/bigan/saved_model/{model_type}"):
            os.makedirs(f"./brain_to_image/bigan/saved_model/{model_type}")
