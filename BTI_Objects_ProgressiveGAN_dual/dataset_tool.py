...
from models.EEGViT_pretrained_ori import (EEGViT_pretrained_ori)
...
class TFRecordExporter:
    ...
    def add_encodedLabels_type(self, labels): #addition of encoded labels
        if self.print_progress:
            print('%-40s\r' % 'Saving Encoded labels Type...', end='', flush=True)
        assert labels.shape[0] == self.cur_images
        with open(self.tfr_prefix + '-rxx.encodedLabelsType', 'wb') as f:
            np.save(f, labels.astype(np.float32))
...
def create_mnist(tfrecord_dir, mnist_dir):
    ...
    #import gzip
    # with gzip.open(os.path.join(mnist_dir, 'train-images.idx3-ubyte'), 'rb') as file:
    #     images = np.frombuffer(file.read(), np.uint8, offset=16)
    # with gzip.open(os.path.join(mnist_dir, 'train-labels.idx1-ubyte'), 'rb') as file:
    #     labels = np.frombuffer(file.read(), np.uint8, offset=8) 
    #Replace import method
    import struct
...
def create_DualObjects(tfrecord_dir, object_dir, fileName, classifierModelPath):
    ...
    encoded_type_labels = np.argmax(encoded_type_labels, axis = 1)
    encoded_type_labels = encoded_type_labels.astype(np.int32)        
    ...
    onehot_encoded_type = np.zeros((encoded_type_labels.size, np.max(encoded_type_labels) + 1), dtype=np.float32)
    onehot_encoded_type[np.arange(encoded_type_labels.size), encoded_type_labels] = 1.0
    ...
    with TFRecordExporter(tfrecord_dir, images.shape[0]) as tfr:
        ...
        tfr.add_encodedLabels_type(onehot_encoded_type[order])
...
def create_Objects(tfrecord_dir, object_dir, fileName, classifierModelPath):
    ...
    images = np.transpose(data['x_train_img'], (0, 3, 1, 2))
    labels = data['y_train']
    class_labels = list(set(labels)) #obtain a list of all unique labels

    # signal_train = data['x_train_eeg']
    # signal_test = data['x_test_eeg']

    # train = np.transpose(data['x_train_img'], (0, 3, 1, 2))
    # test = np.transpose(data['x_test_img'], (0, 3, 1, 2))

    # images = [train, test]
    # signals = [signal_train, signal_test]
    ...
    if config.TfOrTorch == "TF":
        ...
        encoder_model = EEGViT_pretrained_ori()
        encoder_model.load_state_dict(torch.load(classifierModelPath, map_location=device))
        encoder_model.eval() 
...
## New dataset tp create start
    ...
    p = add_command(    'create_DualObjects',   'Create dataset for DualObjects.',
                                            'create_Objects datasets/processed_datasets/tfRecord/Objects datasets/processed_dataset/filter_mne_car/sub-01 000thresh_AllStackLstm_sub-01.pkl')
    p.add_argument(     'tfrecord_dir',     help='New dataset directory to be created')
    p.add_argument(     'object_dir',      help='Directory containing Objects')
    p.add_argument(     'fileName',         help='file to use')
