...
from loss import processSignals
#----------------------------------------------------------------------------
...
# def eegMetrics(run_id, snapshot, log, metrics, num_images, real_passes, minibatch_size=None):
#     # Locate training run and initialize logging.
#     network_pkl = misc.locate_network_pkl(run_id, snapshot)
#     result_subdir = misc.locate_result_subdir(run_id)
...
#     log_file = os.path.join(run_id, log)
#     print('Logging output to', log_file)
#     misc.set_output_log_file(log_file)
...
#     eeg_data_dir = f"Datasets/MindBigData - The Visual MNIST of Brain Digits/{dataset_dir}"
#     data_file = "data_4ch_epoch_filtered_324_0-85test_MindBigData2022_MNIST_EP.pkl"
...
#     classifier_model_path = f"Datasets\MindBigData - The Visual MNIST of Brain Digits\MNIST_EP\models\eeg_classifier_adm5_final.h5"
...
#             for begin in range(0, num_images, minibatch_size):
#                 sampled_encoded = tf.compat.v1.gather(encoded_eegs, tf.compat.v1.random_uniform([minibatch_size], 0, encoded_eegs.shape[0], dtype=tf.compat.v1.int32))
#                 if training_set.label_size > 0:
#                     labels = tf.compat.v1.argmax(labels, axis=1)
#                 else:
#                     labels = tf.compat.v1.zeros([minibatch_size, 0], "float32")
#                 images = Gs.run(sampled_encoded, labels[begin:end], num_gpus=config.num_gpus, out_mul=127.5, out_add=127.5, out_dtype=np.uint8)
#                 if images.shape[1] == 1:
#             generated_samples = Gs.run(encoded_eegs, conditioning_labels,minibatch_size=32)

#         ## predict on GAN
#         validitys, labels = discriminator.predict([encoded_eegs, conditioning_labels],batch_size=32)
