...
def setup_snapshot_image_grid(G, training_set, 
    encoder_model, # Added as now we are having our encoder model used to generate latent
    ...
    # normalized_latents = batch_norm(latents_raw, training=False)
    # normalized_latents = batch_norm_numpy(latents_raw)
    # normalized_latents = latents_raw
    
    # labels_dataset = tf.compat.v1.data.Dataset.from_tensor_slices(labels_tensor)
    # labels_iterator = tf.compat.v1.data.make_one_shot_iterator(labels_dataset)
    ...
    for idx in range(gw * gh):
        x = idx % gw; y = idx // gw
        while True:
            real, _, encoded_signal, encoded_label, encoded_label_type = training_set.get_minibatch_np(1)
            ...
            labels_type[idx] = encoded_label_type
            break

    # Generate latents.
    # latents = misc.random_latents(gw * gh, G) # Rather than noise we will use the EEG signals instead
    # print("Ori Latents type is :", normalized_latents.dtype)

    # print("Ori Labels :", labels.dtype)
    return (gw, gh), reals, labels, encoded_eegs, labels_type
...
def process_reals(x, lod, mirror_augment, drange_data, drange_net):
    with tf.compat.v1.name_scope('ProcessReals'):
        with tf.compat.v1.name_scope('DynamicRange'):
            x = tf.compat.v1.cast(x, tf.compat.v1.float32)
        ...
        if mirror_augment:
            with tf.compat.v1.name_scope('MirrorAugment'):
                ...
                mask = tf.compat.v1.tile(mask, [1, s[1], s[2], s[3]])
                x = tf.compat.v1.where(mask < 0.5, x, tf.compat.v1.reverse(x, axis=[3]))
        with tf.compat.v1.name_scope('FadeLOD'): # Smooth crossfade between consecutive levels-of-detail.
            ...
            x = tfutil.lerp(x, y, lod - tf.compat.v1.floor(lod))
        with tf.compat.v1.name_scope('UpscaleLOD'): # Upscale to match the expected input/output size of the networks.
            s = tf.compat.v1.shape(x)
            factor = tf.compat.v1.cast(2 ** tf.compat.v1.floor(lod), tf.compat.v1.int32)
...
def train_progressive_gan(
    ...
    network_snapshot_ticks  = 20,           # How often to export network snapshots?
    ...
    with tf.compat.v1.device('/gpu:0'):
        if resume_run_id is not None:
            ...
            G = tfutil.Network('G', num_channels=training_set.shape[0], resolution=training_set.shape[1], label_size=training_set.label_size, labels_type_size = training_set.encoded_label_type_size, **config.G)
            D = tfutil.Network('D', num_channels=training_set.shape[0], resolution=training_set.shape[1], label_size=training_set.label_size, labels_type_size = training_set.encoded_label_type_size, **config.D)
            Gs = G.clone('Gs')
    ...
    with tf.compat.v1.name_scope('Inputs'):
        lod_in          = tf.compat.v1.placeholder(tf.compat.v1.float32, name='lod_in', shape=[])
        ...
        reals, labels, encoded_signals, encoded_labels, encoded_labels_type    = training_set.get_minibatch_tf() ## add eeg signal to here
    ...
    for gpu in range(config.num_gpus):
        with tf.compat.v1.name_scope('GPU%d' % gpu), tf.compat.v1.device('/gpu:%d' % gpu):
            ...
            with tf.compat.v1.name_scope('D_loss'), tf.compat.v1.control_dependencies(lod_assign_ops):
                # D_loss = tfutil.call_func_by_name(G=G_gpu, D=D_gpu, opt=D_opt, training_set=training_set, minibatch_size=minibatch_split, reals=reals_gpu, labels=labels_gpu, **config.D_loss)
                 D_loss = tfutil.call_func_by_name(G=G_gpu, D=D_gpu, opt=D_opt, encoded_signals = encoded_signals, encoded_labels= encoded_labels, encoded_labels_type = encoded_labels_type, training_set=training_set, minibatch_size=minibatch_split, reals=reals_gpu, labels=labels_gpu, **config.D_loss)
    ...
    grid_size, grid_reals, grid_labels, grid_latents, grid_labels_type = setup_snapshot_image_grid(G, training_set, encoder_model,  **config.grid)
    sched = TrainingSchedule(total_kimg * 1000, training_set, **config.sched)
    grid_fakes = Gs.run(grid_latents, grid_labels, grid_labels_type, minibatch_size=sched.minibatch//config.num_gpus)
