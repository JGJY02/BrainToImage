...
class TFRecordDataset:
    def __init__(self,
        ...
        self.encoded_label_type_file = encoded_label_type_file

        self._np_encoded_labels        = None
        self._tf_encoded_labels_var     = None
        self._tf_encoded_labels_dataset = None

        self.encoded_label_type_size         = None      # [component]
        self.encoded_label_type_dtype        = None
        self._np_encoded_labels_type        = None
        self._tf_encoded_labels_type_var     = None
        self._tf_encoded_labels_type_dataset = None
        ...
        if self.encoded_label_type_file is None:
            guess = sorted(glob.glob(os.path.join(self.tfrecord_dir, '*.encodedLabelsType')))
            if len(guess):
                self.encoded_label_type_file = guess[0]
        elif not os.path.isfile(self.encoded_label_type_file):
            guess = os.path.join(self.tfrecord_dir, self.encoded_label_type_file)
            if os.path.isfile(guess):
                self.encoded_label_type_file = guess
        ...
        #load encoded labels type
        assert max_label_size == 'full' or max_label_size >= 0
        self._np_encoded_labels_type = np.zeros([1<<20, 0], dtype=np.float32)
        if self.encoded_label_type_file is not None and max_label_size != 0:
            self._np_encoded_labels_type = np.load(self.encoded_label_type_file)
            assert self._np_encoded_labels_type.ndim == 2
        
        if max_label_size != 'full' and self._np_encoded_labels_type.shape[1] > max_label_size:
            self._np_encoded_labels_type = self._np_encoded_labels_type[:, :max_label_size]
        self.encoded_label_type_size = self._np_encoded_labels_type.shape[1]
        self.encoded_label_type_dtype = self._np_encoded_labels_type.dtype.name
        ...
        with tf.compat.v1.name_scope('Dataset'), tf.compat.v1.device('/cpu:0'):
            ...
            self._tf_encoded_labels_var = tf.compat.v1.Variable(tf_encoded_labels_init, name='encoded_labels_var')
            ...
            tf_encoded_labels_type_init = tf.compat.v1.zeros(self._np_encoded_labels_type.shape, self._np_encoded_labels_type.dtype)
            self._tf_encoded_labels_type_var = tf.compat.v1.Variable(tf_encoded_labels_type_init, name='encoded_labels_type_var')
            tfutil.set_vars({self._tf_encoded_labels_type_var: self._np_encoded_labels_type})
            self._tf_encoded_labels_type_dataset = tf.compat.v1.data.Dataset.from_tensor_slices(self._tf_encoded_labels_type_var)
            ...
            for tfr_file, tfr_shape, tfr_lod in zip(tfr_files, tfr_shapes, tfr_lods):
                ...
                if config.dual:
                    dset = tf.compat.v1.data.Dataset.zip((dset, self._tf_labels_dataset, self._tf_signals_dataset, self._tf_encoded_labels_dataset, self._tf_encoded_labels_type_dataset))
                else:
