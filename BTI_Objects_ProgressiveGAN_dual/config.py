...
eeg_dataset_pickle = f"000thresh_AllStack_Transformer_dual.pkl" #"000thresh_AllStackLstm_{category}.pkl" #"934thresh_processed_train_MindBigData2022_MNIST_EP.pkl" #"data_train_MindBigData2022_MNIST_EP.pkl"
result_dir = 'results'

eval_classifier_dir = f"trained_models/classifiers/All/000thresh/Transformer_dual_all_stacked_signals"
...
# train = EasyDict(func='util_scripts.evaluate_metrics', run_id=os.path.join("results", "004-pgan-mnist-preset-v2-1gpu-fp32-GRAPH-HIST"), log='metric-msssim-20k.txt', metrics=['msssim'], num_images=20000, real_passes=1); num_gpus = 1; desc = train.log.split('.')[0] + '-' + str(train.run_id)
# train = EasyDict(func='util_scripts.eegMetrics', run_id=os.path.join("results", "004-pgan-mnist-preset-v2-1gpu-fp32-GRAPH-HIST"), snapshot = 2104 , log='metric-msssim-20k.txt', metrics=['msssim'], num_images=20000, real_passes=1); num_gpus = 1; desc = train.log.split('.')[0] + '-' + str(train.run_id)
