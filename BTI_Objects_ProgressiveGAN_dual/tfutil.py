...
def lerp_clip(a, b, t):
    with tf.compat.v1.name_scope('LerpClip'):
        return a + (b - a) * tf.compat.v1.clip_by_value(t, 0.0, 1.0)
...
#----------------------------------------------------------------------------
# Create tf.compat.v1.Session based on config dict of the form
...
#----------------------------------------------------------------------------
# Initialize all tf.compat.v1.Variables that have not already been initialized.
# Equivalent to the following, but more efficient and does not bloat the tf graph:
#   tf.compat.v1.variables_initializer(tf.compat.v1.report_unitialized_variables()).run()
...
#----------------------------------------------------------------------------
# Set the values of given tf.compat.v1.Variables.
# Equivalent to the following, but more efficient and does not bloat the tf graph:
#   tfutil.run([tf.compat.v1.assign(var, value) for var, value in var_to_value_dict.items()]
...
def autosummary(name, value):
    ...
    if is_tf_expression(value):
        with tf.compat.v1.name_scope('summary_' + id), tf.compat.v1.device(value.device):
            update_op = _create_autosummary_var(name, value)
            with tf.compat.v1.control_dependencies([update_op]):
                return tf.compat.v1.identity(value)
        ...
        if name not in _autosummary_immediate:
            with absolute_name_scope('Autosummary/' + id), tf.compat.v1.device(None), tf.compat.v1.control_dependencies(None):
                update_value = tf.compat.v1.placeholder(tf.compat.v1.float32)
        ...
        for name, vars in _autosummary_vars.items():
            ...
            with absolute_name_scope('Autosummary/' + id):
                ...
                with tf.compat.v1.control_dependencies([avg]): # read before resetting
                    reset_ops = [tf.compat.v1.assign(var, tf.compat.v1.zeros(2)) for var in vars]
...
def _create_autosummary_var(name, value_expr):
    ...
    v = tf.compat.v1.cast(value_expr, tf.compat.v1.float32)
    if v.shape.ndims is 0:
        v = [v, np.float32(1.0)]
    elif v.shape.ndims is 1:
        v = [tf.compat.v1.reduce_sum(v), tf.compat.v1.cast(tf.compat.v1.shape(v)[0], tf.compat.v1.float32)]
    else:
        v = [tf.compat.v1.reduce_sum(v), tf.compat.v1.reduce_prod(tf.compat.v1.cast(tf.compat.v1.shape(v), tf.compat.v1.float32))]
    v = tf.compat.v1.cond(tf.compat.v1.is_finite(v[0]), lambda: tf.compat.v1.stack(v), lambda: tf.compat.v1.zeros(2))
    with tf.compat.v1.control_dependencies(None):
        var = tf.compat.v1.Variable(tf.compat.v1.zeros(2)) # [numerator, denominator]
    update_op = tf.compat.v1.cond(tf.compat.v1.is_variable_initialized(var), lambda: tf.compat.v1.assign_add(var, v), lambda: tf.compat.v1.assign(var, v))
...
#----------------------------------------------------------------------------
# Wrapper for tf.compat.v1.train.Optimizer that automatically takes care of:
...
class Optimizer:
    def __init__(
        ...
        **kwargs):
        ...
        # Init fields.
        self.name               = name
        self.learning_rate      = tf.compat.v1.convert_to_tensor(learning_rate)
    ...
    def register_gradients(self, loss, vars):
        ...
        with tf.compat.v1.name_scope(self.id + '_grad'), tf.compat.v1.device(dev):
            if dev not in self._dev_opt:
                ...
                self._dev_grads[dev] = []
            loss = self.apply_loss_scaling(tf.compat.v1.cast(loss, tf.compat.v1.float32))
            ...
            grads = [(g, v) if g is not None else (tf.compat.v1.zeros_like(v), v) for g, v in grads] # replace disconnected gradients with zeros
    ...
    def apply_updates(self):
        ...
        with absolute_name_scope(self.scope):
            ...
            for dev_idx, dev in enumerate(devices):
                with tf.compat.v1.name_scope('ProcessGrads%d' % dev_idx), tf.compat.v1.device(dev):
                    sums = []
                    for gv in zip(*self._dev_grads[dev]):
                        assert all(v is gv[0][1] for g, v in gv)
                        g = [tf.compat.v1.cast(g, tf.compat.v1.float32) for g, v in gv]
                        g = g[0] if len(g) == 1 else tf.compat.v1.add_n(g)
                        sums.append((g, gv[0][1]))
                    dev_grads[dev] = sums

            # Sum gradients across devices.
            if len(devices) > 1:
                with tf.compat.v1.name_scope('SumAcrossGPUs'), tf.compat.v1.device(None):
                    for var_idx, grad_shape in enumerate(self._grad_shapes):
                        g = [dev_grads[dev][var_idx][0] for dev in devices]
                        if np.prod(grad_shape): # nccl does not support zero-sized tensors
                            g = tf.compat.v1.contrib.nccl.all_sum(g)
                        for dev, gg in zip(devices, g):
                            dev_grads[dev][var_idx] = (gg, dev_grads[dev][var_idx][1])

            # Apply updates separately on each device.
            for dev_idx, (dev, grads) in enumerate(dev_grads.items()):
                with tf.compat.v1.name_scope('ApplyGrads%d' % dev_idx), tf.compat.v1.device(dev):

                    # Scale gradients as needed.
                    if self.use_loss_scaling or total_grads > 1:
                        with tf.compat.v1.name_scope('Scale'):
                            coef = tf.compat.v1.constant(np.float32(1.0 / total_grads), name='coef')
                            coef = self.undo_loss_scaling(coef)
                            grads = [(g * coef, v) for g, v in grads]

                    # Check for overflows.
                    with tf.compat.v1.name_scope('CheckOverflow'):
                        grad_ok = tf.compat.v1.reduce_all(tf.compat.v1.stack([tf.compat.v1.reduce_all(tf.compat.v1.is_finite(g)) for g, v in grads]))
                    ...
                    with tf.compat.v1.name_scope('UpdateWeights'):
                        ...
                        if not self.use_loss_scaling:
                            ops.append(tf.compat.v1.cond(grad_ok, lambda: opt.apply_gradients(grads), tf.compat.v1.no_op))
                        else:
                            ops.append(tf.compat.v1.cond(grad_ok,
                                lambda: tf.compat.v1.group(tf.compat.v1.assign_add(ls_var, self.loss_scaling_inc), opt.apply_gradients(grads)),
                                lambda: tf.compat.v1.group(tf.compat.v1.assign_sub(ls_var, self.loss_scaling_dec))))
            ...
            return tf.compat.v1.group(*ops, name='TrainingOp')
    ...
    def get_loss_scaling_var(self, device):
        ...
        if device not in self._dev_ls_var:
            with absolute_name_scope(self.scope + '/LossScalingVars'), tf.compat.v1.control_dependencies(None):
...
class Network:
    def __init__(self,
        ...
        **static_kwargs):   # Keyword arguments to be passed in to the network construction function.
        ...
        print("Build function name , ",self._build_func_name)
    ...
    def _init_graph(self):
        ...
        # print("Input templates are ", self.input_templates)
    ...
    def get_output_for(self, *in_expr, return_as_list=False, **dynamic_kwargs):
        ...
        with tf.compat.v1.variable_scope(self.scope, reuse=True):
            assert tf.compat.v1.get_variable_scope().name == self.scope
            named_inputs = [tf.compat.v1.identity(expr, name=name) for expr, name in zip(in_expr, self.input_names)]
    ...
    def run(self, *in_arrays,
        ...
        **dynamic_kwargs):          # Additional keyword arguments to pass into the network construction function.
        ...
        if key not in self._run_cache:
            with absolute_name_scope(self.scope + '/Run'), tf.compat.v1.control_dependencies(None):
                in_split = list(zip(*[tf.compat.v1.split(x, num_gpus) for x in self.input_templates]))
                out_split = []
                for gpu in range(num_gpus):
                    with tf.compat.v1.device('/gpu:%d' % gpu):
                        ...
                        if out_shrink > 1:
                            ...
                            out_expr = [tf.compat.v1.nn.avg_pool(x, ksize=ksize, strides=ksize, padding='VALID', data_format='NCHW') for x in out_expr]
                        if out_dtype is not None:
                            if tf.compat.v1.as_dtype(out_dtype).is_integer:
                                out_expr = [tf.compat.v1.round(x) for x in out_expr]
                            out_expr = [tf.compat.v1.saturate_cast(x, out_dtype) for x in out_expr]
                        out_split.append(out_expr)
                self._run_cache[key] = [tf.compat.v1.concat(outputs, axis=0) for outputs in zip(*out_split)]
