...
def embed(x, fmaps, gain=np.sqrt(2), use_wscale=False, size = 10):
    w = get_weight([size, fmaps], gain=gain, use_wscale=use_wscale)
...
def G_paper(
    ...
    **kwargs):                          # Ignore unrecognized keyword args.
    ...
    labels_type_in.set_shape([None, labels_type_size])
    ...
    def embedding_layer(latents, labels, latent_size, labels_type_in = None):
        with tf.compat.v1.variable_scope('embedding_layer'):
            ...
            labels_embedding = embed(labels, latent_size, size =label_size)
            latent_combined = latents * labels_embedding   
        if labels_type_in != None: 

            with tf.compat.v1.variable_scope('embedding_layer_type'):

                labels_type_in = tf.compat.v1.argmax(labels_type_in, axis=1)
                labels_type_in = tf.expand_dims(labels_type_in, axis=1)
                labels_type_in_embedding = embed(labels_type_in, latent_size, size =labels_type_size)

                latent_combined = latent_combined * labels_type_in_embedding
    ...
    combo_in = embedding_layer(latents_in, labels_in, latent_size, labels_type_in)
