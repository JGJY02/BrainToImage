...
parser.add_argument('--classifier_file_path', type=str, help="specific eeg file to use LSTM: All/000thresh/LSTM_all_stacked_signals / CNN: All/000thresh/CNN_all_stacked_signals", default = "trained_models/classifiers/All/000thresh/Transformer_dual_all_stacked_signals" , required=False)
...
parser.add_argument('--output_name', type=str, help="Directory to save processed file", default = "TransformerDual_PROGAN",required=False)
...
dataset = "2022Data"
data_file = f"{config.eeg_dataset_dir}/{config.eeg_dataset_pickle}"
...
for i in class_labels:  ## outer loop per class
    ...
    with tf.Graph().as_default(), tfutil.create_session(config.tf_config).as_default():
        ...
        validitys_true, labels_true_pred, validitys_type_true, labels_type_true_pred  = discriminator.run(true_images, minibatch_size = minibatch_size)
    ...
    history[i] = {'generated':generated_samples,'true':true_images,'valid':validitys,'predicted':labels_pred,'conditioning':conditioning_labels_raw, 'true_labels':labels, \
        'predicted_conditioning_type': labels_type_pred,'conditioning_type': conditioning_labels_type, 'true_conditioning_type': true_conditioning_labels_type, \
...
def save_imgs(images, name, class_label, conditioning_labels, conditioning_type,  predicted_labels, pred_type, real_label, real_type, output_dir):
    ...
    for k, ax in enumerate(axes.flat):
        if k < len(images):
            ...
            #P predicted
            #R True
            ax.set_title(f"C: {int(conditioning_labels[k])}, C_type: {int(conditioning_type[k])}, P: {int(predicted_labels[k])}, P_type: {int(pred_type[k])}, R: {int(real_label[k])}, R_type: {int(real_type[k])}", fontsize=8)
...
for i in class_labels:
    ...
    #For label type
    conditioning_labels_type_array = np.argmax(class_data['conditioning_labels_type'], axis = 1)
    pred_conditioning_labels_type_array = np.argmax(class_data['predicted_conditioning_type'], axis = 1)
    true_labels_type_array = np.argmax(class_data['true_conditioning_type'], axis = 1)
    ...
    for j in range(class_data['generated'].shape[0]):
        ...
        if i == np.argmax(class_data['predicted_conditioning_type'][j]):
        ...
        #PSNR value
        psnr_value = cv2.PSNR(y_true, y_pred)
    ...
    text_to_print = f"Class {i}: mean ssim: {evaluation[i]['average_ssim']:.2f}, mean rmse: {evaluation[i]['average_rmse']:.2f}, mean psnr: {evaluation[i]['average_psnr']:.2f},  \
        mean fsim: {evaluation[i]['average_fsim']:.2f}, mean uiq: {evaluation[i]['average_uiq']:.2f},classification acc: {class_acc:.1%},classification acc: {class_type_acc:.1%}"
