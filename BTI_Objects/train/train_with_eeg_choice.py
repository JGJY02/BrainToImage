...
for epoch in range(epochs):
    ...
    filtered_mnist_idx = np.random.choice(np.where(filtered_mnist_labels)[0], size=batch_size, replace=False)
    # Image labels. 0-9
    img_labels = np.argmax(y_train[filtered_mnist_idx],axis=1)
    # Images
    imgs = x_train[filtered_mnist_idx]
