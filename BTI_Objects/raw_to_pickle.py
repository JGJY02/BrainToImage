...
parser.add_argument('--file', type=str, help="File to package either 1-50 or ALL", default = "48",required=False)
parser.add_argument('--output_root_dir', type=str, help="Directory to output", default = "processed_dataset",required=False)
args = parser.parse_args()

if args.file == "ALL":
    files_to_process = [f"{i:02}" for i in range(13, 51)]
else:
    files_to_process = [args.file]

for file in files_to_process:
    #Declare input path
    input_dir = f"{args.input_root_dir}/sub-{file}/eeg"
    vhdr_file = f"sub-{file}_task-rsvp_eeg.vhdr"
    csv_file = f"sub-{file}_task-rsvp_events.csv"
    ...
    try:
        ...
        output_file = f"sub-{file}_eeg_data.pkl"
        ...
        if "Usecols do not match columns" in str(e):
            print(f"Skipping file {csv_path}: missing one or more required columns {keys}")
