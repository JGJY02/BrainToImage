from models.EEGViT_pretrained import EEGViT_pretrained, EEGViT_pretrained_512, EEGViT_pretrained_512_images
from models.EEGViT import EEGViT_raw
from models.ViTBase import ViTBase
from helper_functions import split
from dataset.EEGEyeNet import EEGEyeNetDataset, MultiClassImageDataset
import torch
...
model = EEGViT_pretrained_512(eeg_data['y_train'].shape[1], eeg_data['y_secondary_train'].shape[1]) # Cognitive Data

vision_model = EEGViT_pretrained_512_images(eeg_data['y_train'].shape[1], eeg_data['y_secondary_train'].shape[1]) # Image data
vision_model.load_state_dict(torch.load('trained_models/Transformer_512_dual_large_image_transformer/eeg_classifier_adm5_final.pth'))
vision_model.eval()  # Set to evaluation mode if you're not training

# EEGEyeNet = MultiClassDataset('./dataset/000thresh_AllStack_Transformer_dual.pkl')
save_path = "trained_models/Transformer_512_dual_large_LatentLossMatching"
...
def train(model, vision_model, optimizer, scheduler = None):
    '''
    ...
    for epoch in range(n_epoch):
        ...
        for i, (inputs, target1, target2, images, index) in tqdm(enumerate(train_loader), desc="Training"):
            ...
            optimizer.zero_grad()
            output1, cog_latent, output2 = model(inputs)
            _, vis_latent, _ = vision_model(images)
            ...
            latent_loss = torch.nn.functional.mse_loss(vis_latent, cog_latent)
            
            loss = loss1 + loss2 + latent_loss
            ...
            epoch_train_loss_latent += latent_loss.item()
        ...
        epoch_train_loss_latent /= len(train_loader)
        ...
        train_losses_latent.append(epoch_train_loss_latent)

        train_accuracies_1.append(epoch_train_acc_1)
        train_accuracies_2.append(epoch_train_acc_2)


        print(f"Epoch {epoch}, Train Results, Total Loss: {epoch_train_loss_total},Class Loss: {epoch_train_loss_1},Type Loss: {epoch_train_loss_2},latent Loss: {epoch_train_loss_latent}  \
        ...
        with torch.no_grad():
            val_loss = 0.0
            total = 0
            for inputs, target1, target2, images, index in val_loader:
                # Move the inputs and targets to the GPU (if available)
                inputs = inputs.to(device)
                target1 = target1.to(device)
                target2 = target2.to(device)
                ...
                epoch_val_loss_latent += latent_loss.item()
        ...
        epoch_val_loss_latent /= len(val_loader)
        ...
        val_losses_latent.append(epoch_val_loss_latent)

        val_accuracies_1.append(epoch_val_acc_1)
        val_accuracies_2.append(epoch_val_acc_2)


        print(f"Epoch {epoch}, Val Results, Total Loss: {epoch_val_loss_total},Class Loss: {epoch_val_loss_1},Type Loss: {epoch_val_loss_2}, Latent loss: {epoch_val_loss_latent}  \
        ...
        with torch.no_grad():
            ...
            for inputs, target1, target2, images, index in test_loader:
                # Move the inputs and targets to the GPU (if available)
                inputs = inputs.to(device)
                target1 = target1.to(device)
                target2 = target2.to(device)
                ...
                epoch_test_loss_latent += latent_loss.item()
        ...
        epoch_test_loss_latent /= len(test_loader)
        ...
        test_losses_latent.append(epoch_test_loss_latent)

        test_accuracies_1.append(epoch_test_acc_1)
        test_accuracies_2.append(epoch_test_acc_2)


        print(f"Epoch {epoch}, Test Results, Total Loss: {epoch_test_loss_total},Class Loss: {epoch_test_loss_1},Type Loss: {epoch_test_loss_2}, Latent Loss: {epoch_test_loss_latent}  \
    ...
    results_dict = {"total_train_loss": train_losses, "total_val_loss": val_losses, "total_test_loss": test_losses, \
        ...
        "train_loss_latent": train_losses_latent, "val_loss_latent": val_losses_latent, "test_loss_latent": test_losses_latent,     \
...
if __name__ == "__main__":
    train(model,vision_model, optimizer=optimizer, scheduler=scheduler)