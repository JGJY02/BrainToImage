from models.EEGViT_pretrained import EEGViT_pretrained, EEGViT_pretrained_512_images
...
model = EEGViT_pretrained_512_images(eeg_data['y_train'].shape[1], eeg_data['y_secondary_train'].shape[1])

# EEGEyeNet = MultiClassDataset('./dataset/000thresh_AllStack_Transformer_dual.pkl')
save_path = "trained_models/Transformer_512_dual_large_image_transformer"
...
def train(model, optimizer, scheduler = None):
    ...
    for epoch in range(n_epoch):
        ...
        for i, (_, target1, target2, inputs, index) in tqdm(enumerate(train_loader), desc="Training"):
        ...
        with torch.no_grad():
            val_loss = 0.0
            total = 0
            for _, target1, target2, inputs, index in val_loader:
            # Move the inputs and targets to the GPU (if available)
                inputs = inputs.to(device)
                target1 = target1.to(device)
                target2 = target2.to(device)
        ...
        with torch.no_grad():
            ...
            for _, target1, target2, inputs, index in test_loader:
            # Move the inputs and targets to the GPU (if available)
                inputs = inputs.to(device)
                target1 = target1.to(device)
                target2 = target2.to(device)
